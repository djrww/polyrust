...
依賴檢討見 [docs/DEPENDENCY_AUDIT.md](docs/DEPENDENCY_AUDIT.md)：`workspace.dependencies` 集中版本、`default-features=false` 最小化、`tokio fs` 等未用特性已移除、`light` 特性實現快速開發編譯（`full` 134→120 行，`llm` 118→86 行 -27%）、`[profile.dev/test]` 加速。
