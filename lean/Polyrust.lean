...
| `Polyrust.Recursion` | **M3/C7**（自遞迴模板） | `fact` 功能規格；`factF` 燃料版（欠燃料唔聲稱結果）；`factF_spec`（n ≤ fuel → 燃料版=結構定義——遞迴模板認證內核）；`fact_pos`（恆正見證）；對應真檔 fact n∈{0,1,3,5,10} CERTIFIED 實證 |
