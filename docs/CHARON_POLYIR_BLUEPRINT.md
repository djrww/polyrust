...
| **C2 直線 + 分支 lowering ✅（2026-09-20）** | `core/llbc_body.rs`（typed body，白名單硬錯；Op token 雙形態、Projection Field、const 去重表全法證實測）→ `core/llbc_lower.rs`（SSA path lowering + continuation-passing switch/loop/call）→ GB 判定（語義表白紙黑字 `core/src/llbc_lower.rs` 頂註） | **matrix basic 10/10 v3↔v4 差分一致**；bad.rs→rustc reject→UNSAT 逐位映射記檔；lazy/eager（cold/warm）時延對表入差分測試報告；**113/113 lib 綠**。殘尾→C3：fib debug GB 48s（triangular fast-path 待做）、Assert 約束接真（C4 panic-freedom）、nested loop control | 3 日完成 |
| **C3 loop + fuel + UNKNOWN ✅（2026-09-20）** | lexit 出邊棧（Break/Continue(depth) 精確路由，**Continue=backedge 法證修正**）+ bounded unroll K=@fuel（預設 3）+ `scan_annotations` 註解雙軌 + V4Opts UNKNOWN 口徑；triangular presolve（SSA 定義鏈健全消去，fib 48s→毫秒級）；新 fixture 形狀法證五連（Opaque body / null fun / Ref / Abort / Str const / Cast / Drop(obj) / nested-const / Deref∘Field collapse） | **loop_unknown（@fuel:1）端到端 UNKNOWN + reason；loop_match 15/15 差分一致；presolve 健全性單測；118/118 lib 綠** | 1 日完成 |
| **C4 calls + contracts + panic-freedom ✅（2026-09-20）** | `core/contract.rs` 合約三級（ExactEq→premise var−c=0（參數白名單安全網）/ CmpAbstract→新鮮 bool b=1 / Opaque→marker；ensure 只收集報告）+ 分析入口參數 eager 綁定 + `V4Outcome.assert_obligations/contract_report`（panic-freedom 檢查面，enum_list 實測 asserts=["Overflow"]）；法證新形狀 `Intrinsic` body / `PlaceMention` statement | **struct/enum 15/15 SAT 差分一致；commercial 10/10 全 SAT（≥6 驗收 ×1.67，enterprise_ide false-UNSAT 修復）；合約端到端兩測（ExactEq premise + 矛盾 UNSAT 硬證據）；128/128 lib 綠、71.6s**（記檔 `docs/C4_CALLS_CONTRACTS.md`） | 1 日完成 |
| **C4 calls / contracts（已被上行 2026-09-20 實績行取代）** | — | — | 完成，見上行 |
| **C5 預設切換 + Lean 補強 ✅（2026-09-20）** | `frontends/llbc` CLI `polyrust-llbc-check`（.rs 預設 v4 真 charon 鏈、.llbc 直達、.poly/`--engine v1` 後備；統一 JSON + exit code 契約；rustc-reject(E-code)→UNSAT 判決）＋ core `analyze_module_from_source`（註解+合約自動接、入口揀選單一權威）；Lean `Polyrust.UnrollSound` 三定理（steps_add 編碼忠實 / steps_mono 抽象單調 / unroll_unsat_reflects UNSAT 反映）#print axioms 僅 propext 且 append 落 Audit.out；`docs/TCB_WHITEPAPER.md` 四層 TCB 邊界圖＋Charon/rustc 章節；README/EVIDENCE C5 段 | **真端到端：bad.rs→Unsat+E0308（rc=1）、ok.rs→Sat+obligations（rc=0）、sqr.llbc 直達、arith.poly→v1；lake build 44 jobs 綠；126+6+2 測試全綠** | 同日完成（C5 kick-off） |
