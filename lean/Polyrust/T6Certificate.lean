...
4. **𝔽_p 上的 1 ∈ I**：給定 D 的模 p 逆元見證，可把 D 縮放為常數 1——
   此定理（`mod_p_one_in_ideal`）現已實現於 `Polyrust.BoolNullstellensatz`
   （v0.1.4 前此處僅有檔首承諾而無實現，屬過度聲稱，已補完）。
...
- 插值與憑證都在**有限點集**（支撐 ≤ n 的布爾點）上陳述；點集以列舉
  `pts` 給出，並假設 `pts.Nodup` 與覆蓋性——列舉由
  `Polyrust.Squarefree.allBits` 提供，其無重複性已證（`allBits_nodup`，
  v0.1.4 補完；此前僅以註解聲稱）。
