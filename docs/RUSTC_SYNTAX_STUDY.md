...
> **WRP-R5 自主推进（2026-09-20）**：`checker.rs`/`constraints.rs` 的 `println!` 按 `rustc` 视为 `Unit`（`Type::Unit`），`minirust` 的 `Call`/`Invoke` 对未定义函数/宏的“强制矛盾”路径增加 `println` 白名单，`v1` 的 `体型别 {} vs ()` 消解，`v1↔v3` 差分清零，`syn` 前端的 `Unit` 与 `Pure` 语义在 `core` 侧同构闭合。
