...
cd lean
lake build            # 預設目標 Polyrust：13 個模組 + 根模組
...
| `Polyrust/Squarefree.lean` | 289 | **T4** | `standard_implies_squarefree`；`toBits/ofBits` 雙射；`squarefree_count`（恰 2ⁿ）；`sublist_chain_length` + `buchberger_extension_bound`（≤ 2ⁿ）+ `no_infinite_sublist_chain` |
...
| **T3(b)** | `resolution_identity`：P_{C∨D} = P_{l∨C}·P_D + P_C·P_{¬l∨D}（**逐點、無條件**）；`learned_preserves_models`/`learned_preserves_polyZero`：被蘊涵的學習子句不改變模型集與零集 | 完整的 P_C ∈ ⟨P_Φ ∪ B⟩ 需要 Nullstellensatz／根式理想理論（本倉庫不含）；Lean 給的是**構造性核心**（消解組合 + 值層守恆） |
...
| **T6** | `one_mem_no_root`：1 ∈ 理想 ⟹ 無 0/1 根（求值同態論證）；`no_root_certificate`、`no_root_poly_certificate`：無公共零點 ⟹ 存在乘子使線性組合處處非零；**借用側** `borrow_one_mem_no_root`＋`borrow_clash_one_mem`／`borrow_assign_one_mem`（衝突系統的 1 有**顯式可讀見證**：`x_i x_j − x_j(x_i−1) − (x_j−1) = 1`） | 「1 ∈ G ⟺ 不可定型」的 ⟸ 方向由 T1 給；⟹ 方向的一般證明需 L0 + T2 的逆否（本倉庫以 0/1 位元直接給出）+ 無根證書 |
